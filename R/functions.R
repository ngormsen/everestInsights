...
AddAcquisitionTimestamp <- function(data, orderTimestampCol, customerIdCol){
  data[, acqTimestamp := min(get(orderTimestampCol)), by = customerIdCol]
}
...
AddMonthlyCohorts <- function(data, orderTimestampCol, acqTimestampCol, custIdCol){
  data[, cohortName := format(get(acqTimestampCol), "%Y-%m")]
  data[, periodName := format(get(orderTimestampCol), "%Y-%m")]
  
  data[, age := 
         (year(get(orderTimestampCol)) - year(get(acqTimestampCol))) * 12 +
         (month(get(orderTimestampCol)) - month(get(acqTimestampCol)))]
  
  data[, period := 
         (year(get(orderTimestampCol)) - year(min(get(orderTimestampCol)))) * 12 +
         month(get(orderTimestampCol)) - month(min(get(orderTimestampCol))) + 1]
...
AddQuarterlyCohorts <- function(data, orderTimestampCol, acqTimestampCol, custIdCol){
  data[, cohortName := LubridateQuarterToPrettyString(
    quarter(get(acqTimestampCol), with_year = T)
  )]
  
  data[, periodName := LubridateQuarterToPrettyString(
    quarter(get(orderTimestampCol), with_year = T)
  )]
  
  data[, age := 
         (year(get(orderTimestampCol)) - year(get(acqTimestampCol))) * 4 +
         (quarter(get(orderTimestampCol)) - quarter(get(acqTimestampCol)))]
  
  data[, period := 
         (year(get(orderTimestampCol)) - year(min(get(orderTimestampCol)))) * 4 +
         quarter(get(orderTimestampCol)) - quarter(min(get(orderTimestampCol))) +1]
...
AddYearlyCohorts <- function(data, orderTimestampCol, acqTimestampCol, custIdCol){
  data[, cohortName := as.character(year(get(acqTimestampCol)))]
  data[, periodName := as.character(year(get(orderTimestampCol)))]
  
  data[, age := year(get(orderTimestampCol)) - year(get(acqTimestampCol))]
  
  data[, period := year(get(orderTimestampCol)) - year(min(get(orderTimestampCol))) + 1]
...
SegmentCohortData <- function(data, filterVariable){
  ...
  values <- unique(data[, get(filterVariable)])
